...
import type { Hit as AlgoliaHit, SearchClient } from 'instantsearch.js/es/types'

export type SvgIconProps = BasePropsWithoutRef<'svg'> & {
  size?: number
  title?: string
  className?: string
  width?: string | number
...
export interface BasePropsWithRef<T> {
  children?: React.ReactNode
  className?: string
  title?: string
  ref?: string
  rootProps?: React.ComponentPropsWithRef<T>
}
...
export type AlgoliaNpmPkgMeta = {
  ...
  computedMetadata: Record<string, unkown>
  created: number
  modified: number
