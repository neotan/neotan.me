...
export default function NpmHubApp() {
  ...
  return (
    <div className="mx-auto w-full flex-1 px-4" suppressHydrationWarning>
      <main className="flex flex-col">
        ...
        >
          ...
          {hasQuery ? (
            <>
              <Hits
                className="rounded-xs relative mt-2 rounded-md"
                classNames={{
                  list: 'space-y-4',
                }}
            ...
            <div className="font-heading flex grow flex-col items-center justify-center space-y-6 text-center text-[clamp(1.1rem,4vw,1.5rem)] text-zinc-500 text-opacity-50">
              <div>🔍</div>
              <div>
...
function CustomHit({ hit }: AlgoliaHitProps) {
  ...
  return (
    <Card
      className={cn(
        'flex flex-col space-y-4 rounded-md border-none p-4 font-light shadow-lg sm:space-y-6 sm:p-8',
    ...
    >
      <div className="flex flex-row flex-wrap items-center gap-2">
        <h2
          ...
          className={cn(
            'flex cursor-pointer items-center space-x-2 rounded-md px-2 py-1 text-lg font-normal text-accent sm:w-auto md:text-2xl',
            {
        ...
        <div className="flex items-center space-x-2">
          ...
          {hit.downloadsLast30Days && (
            <Badge className="gap-2 bg-secondary text-sm font-thin text-foreground hover:bg-secondary" title="Downloads in last 30d">
              <FaDownload />
      ...
      <div className="flex flex-col space-y-6">
        <div className="flex flex-wrap items-center gap-2">
          {hit.owner?.avatar && (
            <Link href={hit.homepage || '#'} target="_blank" className='flex items-center gap-2' title='Homepage'>
              <img
                width="20"
                height="20"
                alt={hit.owner?.name}
          ...
          {hit.version && (
            ...
            >
              <Badge className="gap-2 bg-secondary text-sm font-thin text-foreground hover:bg-secondary">
                <FaTag />
                <div>{hit.version} </div>
                <Dot />
          ...
          {hit.license && (
            <Badge className="gap-2 bg-secondary text-sm font-thin text-foreground hover:bg-secondary" title={hit.license}>
              <FaBalanceScale />
        ...
        {!isNilOrEmpty(hit.keywords) && (
          <div className="flex flex-wrap items-center gap-2 opacity-80">
            {pipe(
              uniq<string>,
              map(keyword => (
                <Link href={`/apps/npmhub?q=${keyword}`} key={keyword}>
                  <Badge key={keyword} className="gap-2 bg-secondary text-sm font-thin text-foreground hover:bg-secondary">
                    {keyword}
                  </Badge>
                </Link>
              )),
            )(hit.keywords as string[])}
          </div>
        )}
